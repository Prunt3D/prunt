...
[ "$PRUNT_DO_GNATCOV" = true ] && alr exec -P2 -- /usr/local/gnat/bin/gnatcov instrument --level=stmt+mcdc+gexpr --dump-trigger=manual --projects=prunt.gpr --projects=tests.gpr --no-subprojects --ada=2022

# Build testsuite with instrumented code.
[ "$PRUNT_DO_GNATCOV" = true ] && alr build --development -- --src-subdirs=gnatcov-instr --implicit-with=gnatcov_rts
...
[ "$PRUNT_DO_GNATCOV" = true ] && alr exec -P2 -- /usr/local/gnat/bin/gnatcov coverage --annotate=report --output-dir=gnatcov_out --level=stmt+mcdc+gexpr --projects=prunt.gpr --no-subprojects *.srctrace
[ "$PRUNT_DO_GNATCOV" = true ] && alr exec -P2 -- /usr/local/gnat/bin/gnatcov coverage --annotate=shtml+ --output-dir=gnatcov_out --level=stmt+mcdc+gexpr --projects=prunt.gpr --no-subprojects *.srctrace
[ "$PRUNT_DO_GNATCOV" = true ] && alr exec -P2 -- /usr/local/gnat/bin/gnatcov coverage --annotate=xcov+ --output-dir=gnatcov_out --level=stmt+mcdc+gexpr --projects=prunt.gpr --no-subprojects *.srctrace
