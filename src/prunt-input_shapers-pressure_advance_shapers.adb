...
package body Prunt.Input_Shapers.Pressure_Advance_Shapers is
   ...
   begin
      return
        (Input_Offset          => -Half_Smooth_Time - (if Half_Smooth_Time = 0 then 0 else 1),
         Extra_End_Time        => Half_Smooth_Time + (if Half_Smooth_Time = 0 then 0 else 1),
         Buffer_Size           => Half_Smooth_Time * 2 + 1,
         Buffer                => (others => Start_Position),
         Current_Buffer_Index  => 0,
         Previous_Outputs      => (others => Start_Position),
   ...
   begin
      ...
      if This.Half_Smooth_Time = 0 then
         return Step + This.Pressure_Advance_Time * Vel;
      else
         This.Buffer (Right) := Step + This.Pressure_Advance_Time * Vel;
