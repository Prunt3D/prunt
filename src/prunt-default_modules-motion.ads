...
private
   ...
   protected type Module_Instance is new My_Modules.Module_Instance with
      procedure Initialize
        (Config_In : User_Config; Status_Emitter_In : My_Modules.Status_Emitter_Shared_Pointers.Ref);
      ...
      --
      --  Prunt contains a setting under Prunt > Replace G0 with G1 which will replace all G0 commands with G1 commands
      --  if set. This emulates the behaviour seen in some other motion controllers, including Marlin. This setting is
      --  off by default.
      --
      ...
      --  Perform a counter-clockwise arc move in the XY plane. The endpoint is specified by X and Y, and the arc
      --  centre is specified by I and J offsets from the current position.
      --
      --  If just one of X or Y is specified then the command will act as if the value of that parameter were zero in
      --  relative movement mode, as with G0 and G1. If neither X nor Y are specified, a complete circle is performed.
      --
      ...
      with Annotate => (Prunt_Config, Gcode_Command, "G3");
      --  Perform a counter-clockwise arc move in the XY plane. The endpoint is specified by X and Y, and the arc
      --  centre is specified by R as the arc radius.
      --
      --  If just one of X or Y is specified then the command will act as if the value of that parameter were zero in
      --  relative movement mode, as with G0 and G1. If neither X nor Y are specified, a complete circle is performed.
      --
      ...
      procedure Retract (Planner : Planner_Interface'Class)
      ...
      procedure Recover (Planner : Planner_Interface'Class)
      ...
      procedure Millimeter_Units (Planner : Planner_Interface'Class)
      with Annotate => (Prunt_Config, Gcode_Command, "G21");
      --  Does nothing. Provided for compatibility with other motion controllers where G21 sets the units to
      --  millimetres. Prunt always uses millimetres as the unit for g-code.

      procedure Report_Stored_Positions (Planner : Planner_Interface'Class)
      ...
      --
      --  Without axis parameters, all axes are restored. If axis parameters are included, only those axes are
      --  restored.
      --
      ...
      procedure Absolute_Positioning (Planner : Planner_Interface'Class)
      with Annotate => (Prunt_Config, Gcode_Command, "G90");
      --  Set the printer to absolute positioning mode. In this mode G0 and G1 specify absolute coordinates. M83
      --  overrides this behaviour for the E axis. This command acts as-if a M82 command is run at the same time,
      --  meaning that M83 must be called again to set the extruder to relative mode if it was called previously.

      procedure Relative_Positioning (Planner : Planner_Interface'Class)
      with Annotate => (Prunt_Config, Gcode_Command, "G91");
      --  Set the printer to relative positioning mode. In this mode G0 and G1 specify relative coordinates. M82
      --  overrides this behaviour for the E axis. This command acts as-if a M83 command is run at the same time,
      --  meaning that M82 must be called again to set the extruder to absolute mode if it was called previously.
      ...
      --  Set the current position to be used by other g-code commands. This does not change how other parts of Prunt
      --  see the position, so features such as bounds still function as expected.

      procedure E_Axis_Absolute (Planner : Planner_Interface'Class)
      with Annotate => (Prunt_Config, Gcode_Command, "M82");
      --  Set the E axis to absolute positioning mode, overrides G90. Cleared by G90.

      procedure E_Axis_Relative (Planner : Planner_Interface'Class)
      ...
      procedure Set_Auto_Retract
        (Planner : Planner_Interface'Class;
         S       : Gcode_Optional_Float
         --  Enable auto-retract when non-zero, disable when zero.
         )
      with Annotate => (Prunt_Config, Gcode_Command, "M209");
      --  Enable or disable automatic firmware retraction. When enabled, all reversed E-only moves are treated as
      --  retractions using the settings from the last M207 command, and forward E-only moves following a retraction
      --  are treated as recoveries using the settings from the last M207 and M208 commands. This is intended for use
      --  with slicers that do not generate G10 and G11 commands.

      procedure Set_Feedrate_Percentage
        (Planner : Planner_Interface'Class;
         S       : Gcode_Optional_Float
         --  Feedrate percentage. Not modified if not specified.
         )
      with Annotate => (Prunt_Config, Gcode_Command, "M220");
      --  Set the feedrate percentage applied to all G0 and G1 move commands. A value of 100 means no scaling.
      --  Reports the current feedrate percentage if S is not specified.
      ...
      --  Set the flow percentage applied to all E axis moves. A value of 100 means no scaling. This scales the
      --  amount of filament extruded per move.
   private
      Config         : User_Config;
