               ...
               when Lead_Screw =>
                  declare
                     ...
                     Denominator          : constant Dimensionless :=
                       Distance_Config.Lead_Screw.Gear_Ratio.Denominator;
                     Full_Steps           : constant Dimensionless :=
                       Distance_Config.Lead_Screw.Full_Steps_Per_Rotation;
                  begin
                  ...
                  begin
                     Config.Steppers (S) :=
                       (Kind               => TMC2240_UART_Kind,
                              ...
                              then 0
                                ...
                                TMC_Types.TMC2240.Global_Scaler_Type
                                  (Dimensionless'Max
                                     (32.0,
                                      Dimensionless'Floor
                                        (Dimensionless (TMC_Data.Run_Current / (1.0 * amp)) * 256.0)))),
                           Reserved      => 0),
                        IHOLD_IRUN         =>
                          (I_Hold       =>
                             (if TMC_Data.Enabled
                        ...
                        IRUN_During_Homing =>
                          (if TMC_Data.Enabled
                           then
                             TMC_Types.Unsigned_5
                               (Dimensionless'Max
                                  (0.0, Dimensionless'Floor (TMC_Data.IRUN_During_Homing * 32.0 - 1.0)))
                           else 0),
                        TPOWERDOWN         =>
                          (T_Power_Down =>
                        ...
                        PWMCONF            =>
                          (PWM_OFS            =>
                             (if TMC_Data.StealthChop2.Kind = Enabled
                              then TMC_Data.StealthChop2.Enabled.PWM_OFS
                              else 29),
                           PWM_Grad           =>
                             (if TMC_Data.StealthChop2.Kind = Enabled
                              then TMC_Data.StealthChop2.Enabled.PWM_GRAD
                              else 0),
                           PWM_Freq           =>
                             (if TMC_Data.StealthChop2.Kind = Enabled
                           ...
                           PWM_Reg            =>
                             (if TMC_Data.StealthChop2.Kind = Enabled
                              then TMC_Data.StealthChop2.Enabled.PWM_REG
                              else 4),
                           PWM_Lim            =>
                             (if TMC_Data.StealthChop2.Kind = Enabled
                     ...
                     if not TMC_Data.Enabled then
                        ...
                        Config.Steppers (S).CHOPCONF.FD3 :=
                          TMC_Types.Unsigned_1 (TMC_Data.CHM.Constant_Off_Time.TFD / 8);
            ...
            case Data.Thermistors (T).Thermistor_Kind.Kind is
               when Disabled =>
                  Config.Thermistors (T) :=
                    (Kind => Disabled_Kind, Minimum_Temperature => <>, Maximum_Temperature => <>);
                     ...
                     when Always_On =>
                        Config.Fans (F) :=
                          (Kind                    => Always_On_Kind,
                           ...
                           Always_On_PWM           =>
                             Data.Fans (F).Fixed_Switching.Control_Method.Always_On.Duty_Cycle,
