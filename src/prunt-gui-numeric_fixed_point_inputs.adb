...
package body Prunt.GUI.Numeric_Fixed_Point_Inputs is
   ...
   function Nice_Image (Number : T) return UXString is
      function T_Image is new Fixed_Point_Image (T);
      Raw : constant String := T_Image (Number, Fore => 1, Aft => T'Aft, Exp => 0);
   begin
      for I in reverse Raw'Range loop
         if Raw (I) /= '0' then
            if Raw (I) = '.' then
               if I - Raw'First > T'Aft + T'Fore + 1 then
