...
package body Prunt.Config is
   ...
   protected body Config_File is
      ...
      function Get_Element_By_Path (Path : DOM.Core.DOM_String) return DOM_String is
         Current_Node : DOM.Core.Node := XML_Reader.Get_Tree;
      begin
         if Path'Length < 3 or else Path (Path'First) /= "/" or else Path (Path'Last) /= "/" then
            raise Constraint_Error with "Invalid path.";
         end if;

         for Next_Node_Name of GNAT.String_Split.Create (Path (Path'First + 1..Path'Last), "/") loop
            declare
               Current_Node_List : DOM.Core.Node_List := DOM.Core.Nodes.Child_Nodes (Current_Node);
            begin
               Current_Node := null;
               for N of Current_Node_List loop
                  if DOM.Core.Nodes.Local_Name (N) = Next_Node_Name then
                     if Current_Node /= null then
                        raise Constraint_Error with "Duplicate node name: " & DOM.Core.Nodes.Local_Name (N);
                     else
                        Current_Node := N;
                     end if;
                  end if;
